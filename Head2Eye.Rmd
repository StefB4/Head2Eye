...
REFERENCE_DATA_PATH = "./post_recorded_reference_data/Build/All_Failed/"
...
```{python}
print(reference_scenedata_files["Westbrueck"]["events"])
print(reference_eyetracking_files["Westbrueck"]["full_df"]["UnixTimeStamp"][100])
print(reference_eyetracking_files["Westbrueck"]["full_df"]["UnixTimeStamp"][101])
print(reference_eyetracking_files["Westbrueck"]["full_df"]["UnixTimeStamp"].iloc[-1])
```

```{python}
# Todo 
# cut events from reference input files
# rebase timestamps to start at 0 
# per path until event 
# resample/ add datapoints in 0.01s intervals (probably, as 90 Hz is roughly 0.011s and 90 Hz is max frequ) 
```
